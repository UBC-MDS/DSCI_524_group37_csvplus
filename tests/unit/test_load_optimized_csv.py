...
def test_no_category_cols_excludes_column(sample_csv):
    """Test that no_category_cols excludes columns from categorical conversion."""
...
def test_category_threshold_controls_conversion(tmp_path):
    """Test that category_threshold affects which columns become categorical."""
    # 20% unique (2 unique / 10 total)
    df = pd.DataFrame({"col": ["A", "A", "A", "A", "A", "B", "B", "B", "B", "B"]})
